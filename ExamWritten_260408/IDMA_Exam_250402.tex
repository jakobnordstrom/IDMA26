...
\begin{problem}
  (80 p)
  In this problem we consider linked lists sorted in increasing order.
  ...
  \end{itemize}
%%%
  The link is initialized with two sentinel records
  \verb+head+
  and
  \verb+tail+
  such that
  ...
  \item
    ...
    \verb+tail.prev+ $=$ \verb+NULL+,
    and
    \verb+tail.next+ $=$ \verb+NULL+;
  
  \end{itemize}
%%%
  This ensures that \verb+head+ will always be the first element and
  \verb+tail+ will always be the last element in the linked list.
  ...
  \end{enumerate}
%%%
  Note that there is one spoiler piece of code that does none of the above.
  You receive 10 points for each correct match and 10 more points for a brief but convincing
  explanation
...
- lookup in a doubly linked list
- and a spoiler
and ask to match the code snippets to these alternatives (with explanations).
All code snippets would "return" something just to obfuscate a little bit.
10 p for correct match, 10 p for brief explanation
