...
\begin{problem}
  ...
  really trust calculators, so he would very much like to see a clean
...
\begin{problem}
  ...
  \begin{itemize}
  \item
    \verb+data+: the data stored (which we will not care about, as usual),
    
  \item
    \verb+key+: used for sorting elements in the list.
    
  \item
    \verb+prev+: link to previous element.
    
  \item
    \verb+next+: link to next element.
    
  \end{itemize}
%%%
  The lists are sorted in increasing order based on
  \verb+key+ values.
  We assume that distinct records have distinct
  \verb+key+ values, so that every value identifies a unique record.
...
%%%
  ...
  in the list since
  \verb|-INFINITY|$= - \infty$ is smaller than any \verb|key|
  and
  \verb+tail+ will always be the last element in the linked list since
  \verb|+INFINITY|$= +\infty$ is larger than any \verb|key|.

  Below you find four pseudocode functions for operating on the list.
  Please identify which piece of code:
  \begin{enumerate}
  \item
...
%%%
  ...
  above.  You receive 10 points for each correct match and 10 more
  points for a brief but convincing explanation why the piece of code
  does what you claim it does (or for why the spoiler piece of code
  does something else.
...
\begin{problem}
  (70 p)
  In this problem we want to study heaps and how they can be used for sorting.
...
\begin{subproblem}%
  ...
  {(50 p)}
  {\ignorespaces}
  Run heapsort by hand on the array
%%%
  $
  A =
  [ 11, 3, 19, 5, 17, 2, 13, 7 ]
...
%%%
  ...
  algorithm.
  Specifically,
  show how the original heap is built, and how the first two numbers
  in the array are sorted into their correct final positions.
...
%
  ...
  where the input is a max-heap but Jakob's algorithm fails to find the
  minimal element. 
\end{subproblem}
...
%%% d: 40 p
  ...
  Start with small directed graph on 6 vertices
  
  (a) Say that the relation E(u,v) holds for two vertices u and v in the graph if there is a directed edge from u to v.
  Please present the matrix representation of this relation
  
  
  (b) Take the reflexive closure R of (a).
  What is this? Give matrix representation.
  
  (c) Take the transitive closure T of the relation R in (b)
  What is this? Give matrix representation.
  
  (d)
  Define new relation S(u,v) such at S(u,v) holds precisely when
  R(u,v) and R(v,u) both hold.
  ...
  this type of relation when performing the steps above.
  
%%%%
%%%% WILL IT JUST BE TOO MUCH TO ASK THEM TO RUN SSC?
%%%%
%   
...
\begin{problem}%
  (80 p)
  In this problem we want to count the number of ways of tiling a
  $2 \times n$ grid
  with
  $2 \times 1$ tiles (that can be rotated so that they have either
  ...
  \begin{subproblem}
    ...
    \noindent
    \emph{Hint:}
    We want to find a nice formula for this number
    in terms of concepts we have learned in class.
  \end{subproblem}
...
%%%
  ...
  in counting the number of multisets of size $r$.  Jakob has a new
  idea for showing that the number of multisets is
  $\binom{n + r - 1}{n-1}$ by translating every multiset to a standard
  subset of $[n + r - 1]$ of size $n-1$ as follows:
\begin{verbatim}
PSEUDO-CODE HERE
\end{verbatim}
  ...
  \begin{subproblem}%
    ...
    multisets of size $r = 6$.  Describe what subset the multiset
    $[1, 1, 2, 5, 5, 5]$ is translated to by Jakob's algorithm.
  \end{subproblem}
...
\end{solution}
  ...
  \begin{subproblem}%
    ...
    {\ignorespaces}
%%%
    Is Jakob's new algorithm correct? Can you prove that Jakob's
    algorithm provides a
    translation that shows that the number of multisets of size $r$ from
    a universe of size $n$ is $\binom{n + r - 1}{n-1}$? Or if there is an
    ...
    fact incorrect, then just one small counter-example is sufficient.
  \end{subproblem}
...
\end{solution}
  ...
  \begin{subproblem}%
    ...
    subset $S$ by the method we studied in class and is false
    otherwise. 
%%%
    What properties does this predicate $T_{n,r}$ have? Please indicate
    for all alternatives below whether $T_{n,r}$ has this property or
    not. For each property, you get points if you give a correct
    answer whether $T_{n,r}$ has the property or not, a deduction for
    every wrong answer, and no points and no deduction if you do not
